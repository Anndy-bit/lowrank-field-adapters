...
% Version para SCPE (Scalable Computing: Practice and Experience), clase B
% Publindex, ISSN 1895-1767. Portado desde main.tex (formato IEEEtran) al
% formato obligatorio scpe.cls. Contenido identico salvo: preambulo,
% bloque de titulo/autores, abstract (recortado a 200-250 palabras, sin
% citas/ecuaciones/referencias cruzadas, regla de SCPE), y la unica tabla
% ancha (table* -> table, ya no hace falta en una columna).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{\nmf{} (Neural Manifold Flow).} Existing residual adapters apply a static perturbation, $h' = h + \text{MLP}(h)$, a single-step translation. \nmf{} instead defines a continuous deformation via a Neural ODE~\cite{chen2018neuralode}, $dh(t)/dt = f_\theta(h(t), t)$, solved with fixed-step RK4 integration, letting representations follow curved trajectories through the latent manifold that a discrete single-step adapter cannot produce.
...
For scale: full fine-tuning of this architecture would train 8{,}030M parameters (100\%); LoRA $r{=}8$ on all linear layers trains $\sim$16.8M (0.21\%); LoRA $r{=}64$ trains $\sim$134M (1.67\%). S\textsuperscript{3}'s 4.83M (0.060\%) is reported here for context, not as a ranking against other PEFT methods.
...
The mathematical structure of S\textsuperscript{3} enables three training acceleration techniques, each one a direct consequence of the spectral/ODE formulation itself. Each comes with a formal guarantee of zero or negligible quality loss.
...
\textbf{Hardware.} A single NVIDIA GTX 1050, 3.94\,GB usable VRAM, streamed layer-by-layer from local NVMe via a disk-streaming loader; no cloud hardware was used for any \sthree{} or LoRA number reported here.
...
\textbf{\sthree{} hyperparameters.} SVMO: $k=128$, $H=32$, $\alpha=0.3$; NMF: $d_b=8$, $T=1.0$, $N=4$, RK4; STB: $\beta=0.5$. AdamW, lr $10^{-3}$, fp16 AMP, gradient checkpointing, layer-major streaming.
...
\begin{table}[t]
\centering
\caption{Zero-shot accuracy, $n{=}500$ examples/task, Qwen2.5-7B-Instruct. LoRA is shown at two seeds (42, 43); \sthree{}'s second-seed evaluation is not yet available (\S\ref{sec:stats}).}
\label{tab:main}
\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}lcccc@{}}
...
The two LoRA seeds agree to within 0.012 on every task (ARC-Challenge is the widest spread; four of seven tasks agree to within 0.006). LoRA's numbers are included in \Cref{tab:main} as a familiar reference point for these tasks, not as a baseline \sthree{} is being benchmarked against. \sthree{}'s own accuracy is ahead of LoRA's range on BoolQ and PIQA, behind on ARC-Challenge and OpenBookQA, and close to it on the rest. \sthree{} trains 3.90M parameters, versus 20.2M for LoRA rank-8. \sthree{}'s own second seed is not yet available for this table (\S\ref{sec:stats}), so we cannot yet say whether its single run is as stable as LoRA's two.
...
Empirically (\Cref{sec:experiments}), on seven zero-shot lm-eval-harness tasks, \sthree{} trains 3.90M parameters and reaches the downstream accuracy ranges reported in \Cref{tab:main}, with LoRA rank-8 included there as reference context. An ablation over coupled operator pathways shows the spectral pathway (SVMO+STB) matching the flow pathway (NMF) with 15\% fewer parameters at low budget, and combining both beating either alone at matched total budget. A solver-invariance sweep supports that NMF's behavior barely depends on the RK4 step count it was trained under, evidence of a genuine continuous flow rather than a step-count-specific shortcut. The ablation results above are backed by two-seed data (\Cref{sec:stats}); \sthree{}'s own downstream numbers in \Cref{tab:main} are still a single run, pending its second-seed evaluation, and should be read as such. \sthree{}'s intended audience is stated explicitly in \Cref{sec:motivation}: users for whom on-demand cloud GPUs are not actually a substitute. Code, pre-computed SVD factors, and experimental configurations will be released alongside publication.
