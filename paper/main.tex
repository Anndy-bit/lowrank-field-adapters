...
\author{
    \IEEEauthorblockN{Anonymous Researcher%Nicol\'as Andrey Orozco Giraldo%
    }
    \IEEEauthorblockA{%
...
\begin{abstract}
Fine-tuning large language models (LLMs) typically demands 8--16\,GB of GPU memory even with parameter-efficient methods such as LoRA, thereby excluding researchers limited to consumer-grade hardware. We introduce \sthree{} (Spectral-Spatial-Smooth), a novel parameter-efficient fine-tuning (PEFT) paradigm capable of adapting 7B-scale models on GPUs with as little as 2\,GB VRAM. \sthree{} combines three mathematically novel operators with no direct precedent in the PEFT literature: (1)~\textbf{Singular Value Modulation Operator} (\svmo), which modulates each singular value of frozen weight matrices pointwise via a learned nonlinear function---keeping singular vectors frozen and requiring only $O(H^2)$ parameters per matrix, independent of model dimension; (2)~\textbf{Neural Manifold Flow} (\nmf), the first application of continuous-depth Neural ODEs to LLM fine-tuning, deforming latent representations along smooth trajectories via a bottleneck velocity field integrated with fixed-step RK4; and (3)~\textbf{Spectral Transport Bridge} (\stb), a cross-attention coupling mechanism that transports latent representations through the spectral basis of weight matrices, creating bidirectional information flow between weight-space and representation-space adaptations. Together, these three operators constitute a mutually reinforcing adaptation stack totaling only 4.83M parameters ($0.060\%$ of a 7B model) and achieves a peak VRAM of $\sim$345\,MB. We establish ten formal theoretical results: an explicit $O(1/\sqrt{H})$ approximation rate for \svmo; uniform gradient stability via $\tanh$ saturation; corrected Gr\"onwall trajectory bounds for \nmf; RK4 integration guarantees; a mutual information lower bound proving that \stb{} creates non-zero coupling between adaptation spaces; local convergence acceleration through spectral preconditioning; a non-vacuous PAC-Bayes generalization bound---the first such guarantee for PEFT at 7B scale; exact gradient preservation under spectral compression (SGC); linear-in-epoch error accumulation for progressive ODE integration (NFR); and provably non-zero mutual information under stochastic STB bypass (SBS). [EXP: Empirical validation on Qwen2.5-7B, Mistral-7B-v0.3, and Llama-3-8B across five benchmarks (MMLU, HellaSwag, ARC-Challenge, GSM8K, AlpacaEval~2.0) against LoRA, QLoRA, and full fine-tuning is underway. Preliminary VRAM measurements confirm peak usage $\leq$ 350\,MB. All [EXP:\ldots] markers will be replaced with real experimental data.]
\end{abstract}
...
Fine-tuning large language models for downstream tasks is the dominant paradigm in applied NLP~\cite{hu2021lora,dettmers2023qlora}. However, full fine-tuning of a 7--8B parameter model demands over 60\,GB of GPU memory---hardware accessible only via expensive data-center accelerators (NVIDIA A100, H100). Parameter-efficient fine-tuning methods such as LoRA~\cite{hu2021lora}, QLoRA~\cite{dettmers2023qlora}, DoRA~\cite{liu2024dora}, and VeRA~\cite{kopiczko2024vera} substantially reduce this burden, yet even the most frugal among them require 4--8\,GB VRAM. This excludes the vast majority of consumer GPUs, including the widely available NVIDIA GTX 1050 (2--4\,GB). This creates a \textbf{hardware barrier}: independent researchers, students, and laboratories in resource-constrained settings are effectively excluded from adapting state-of-the-art open-weight LLMs to their domains of interest.

We address this barrier by redefining the \textit{floor} of attainable VRAM consumption. Our objective is not an incremental reduction, but a qualitative shift: enabling 7B-scale fine-tuning within a \textbf{2\,GB VRAM budget}, thereby opening LLM adaptation to virtually any computer equipped with a discrete GPU.
...
Together, these three operators constitute a mutually reinforcing adaptation stack totaling only \textbf{4.83M trainable parameters} ($0.060\%$ of a 7B model), achieving a peak VRAM of $\sim$\textbf{345\,MB}---comfortably within the 2\,GB budget of a GTX 1050.
...
Residual adapters that modify hidden states via $h' = h + \text{MLP}(h)$ have been extensively studied~\cite{houlsby2019adapters,pfeiffer2020adapterfusion}. Recent work extends this to frozen LLMs: TALAN~\cite{talan2026} uses task-aligned latent adaptation networks, and Liu~\cite{liu2026decodable} applies residual-stream steering with multi-layer MLPs. These methods apply \textit{single-step static perturbations}. In contrast, \nmf{} substitutes the static MLP with a continuous ODE flow, enabling curved trajectories and explicitly time-dependent dynamics---a structural increase in expressivity at equivalent parameter counts.

While optimal transport has been applied to representation alignment~\cite{abdullaev2026ot}, it has not been used to couple weight-space and representation-space adaptations during fine-tuning. Our \stb{} introduces a spectral cross-attention channel bridging these two domains, a mechanism without direct precedent in the PEFT literature.
...
\textbf{Combined impact (GTX 1050, LOW tier).} SGC reduces internal GPU gradient traffic $32\times$; NFR saves $\sim 33\%$ NMF FLOPs over 3 epochs; SBS with $p_{\text{STB}}=0.3$ eliminates $\sim 70\%$ STB FLOPs. All three preserve training quality---guaranteed by Theorems~\ref{thm:sgc}--\ref{thm:sbs}. Estimated wall-clock: $\sim 30$h baseline $\to$ $\sim 25$--$27$h on GTX 1050.
...
\toprule
\textbf{Method} & \textbf{MMLU} & \textbf{HSwag} & \textbf{ARC-C} & \textbf{GSM8K} & \textbf{AlpEval} \\
\midrule
Base model & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] \\
\sthree{} \smash{(ours)} & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] \\
LoRA $r{=}8$ & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] \\
LoRA $r{=}64$ & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] \\
QLoRA $r{=}8$ & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] \\
Full FT & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] \\
\bottomrule
\end{tabular}
\end{table}

\textbf{[EXP: Statistical analysis.]} $n{=}3$ independent runs per configuration (different seeds). Primary test: two-sided Welch's $t$-test for \sthree{} vs.\ LoRA $r{=}8$. Non-inferiority margin: $\Delta_{\mathrm{NI}}{=}1.0$ pp. Effect size via Cohen's $d$. Bootstrap 95\% CI (10,000 resamples).
...
\begin{tabular}{@{}lccr@{}}
\toprule
\textbf{Method} & \textbf{VRAM} & \textbf{Params} & \textbf{Time} \\
\midrule
\sthree{} (ours) & [EXP: 345\,MB] & 4.83M & [EXP: 30h] \\
LoRA $r{=}8$  & [EXP: 8\,GB]   & 16.8M  & [EXP: 0.5h] \\
QLoRA $r{=}8$ & [EXP: 4.5\,GB] & 16.8M  & [EXP: 2h]   \\
LoRA $r{=}64$ & [EXP: 10\,GB]  & 134M   & [EXP: 0.8h] \\
Full FT       & 60\,GB         & 8.03B  & [EXP: 2h]   \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Training acceleration via Theorems 8--10.} Three structural properties of S\textsuperscript{3} enable formal training speedups with zero quality loss:
\begin{itemize}[leftmargin=*]
    \item \textbf{SGC (Theorem~8)}: The gradient compression $g \cdot U_k U_k^T$ reduces internal GPU gradient memory traffic $32\times$ (from $B \cdot d_{\text{out}}$ to $B \cdot k$ per layer). This lowers memory bandwidth pressure, translating to $\sim$5\% wall-clock reduction on bandwidth-constrained GPUs.
    \item \textbf{NFR (Theorem~9)}: Progressive ODE integration ($N_1=2$, $N_e=4$ for $e \geq 2$) reduces NMF FLOPs by $\approx 33\%$ over 3 epochs, saving $\sim$50 min on GTX 1050.
    \item \textbf{SBS (Theorem~10)}: Stochastic STB bypass with $p_{\text{STB}}=0.3$ eliminates $\sim 70\%$ of STB FLOPs, saving $\sim$2.2 hours on GTX 1050.
\end{itemize}
\textbf{Combined effect:} baseline $\sim$30h $\to$ $\sim$25--27h on GTX 1050, a 10--17\% wall-clock reduction, while preserving training quality via formal guarantees (exact gradient equality for SGC, linear error accumulation for NFR, $I_{\text{SBS}} > 0$ for any $p_{\text{STB}} > 0$ for SBS).
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Configuration} & \textbf{MMLU} & \textbf{HSwag} & \textbf{VRAM} \\
\midrule
Full \sthree{} & [EXP: xx.x] & [EXP: xx.x] & [EXP: 345] \\
$-$ \stb{}    & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] \\
$-$ \nmf{}    & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] \\
$-$ \svmo{}   & [EXP: xx.x] & [EXP: xx.x] & [EXP: xx.x] \\
...
\textbf{[EXP: Optional scaling experiment.]} Test S\textsuperscript{3}-small ($k{=}64$, $H{=}16$, $d_b{=}4$, $\alpha{=}0.2$, $\beta{=}0.3$, $N{=}2$, $T{=}0.5$) and S\textsuperscript{3}-large ($k{=}256$, $H{=}64$, $d_b{=}16$, $\alpha{=}0.5$, $\beta{=}0.7$, $N{=}8$, $T{=}2.0$).
...
We established ten formal results: an explicit $O(1/\sqrt{H})$ approximation rate for SVMO (Theorem~\ref{thm:svmo-approx}); uniform gradient stability via $\tanh$ saturation (Theorem~\ref{thm:svmo-grad}); corrected Gr\"onwall trajectory bounds proving linear-in-$T$ error growth (Theorem~\ref{thm:nmf-traj}); RK4 integration guarantees with $\varepsilon_{\text{ODE}} \leq 6.3\times 10^{-6}$ (Theorem~\ref{thm:nmf-rk4}); a mutual information lower bound proving that STB creates non-zero coupling (Theorem~\ref{thm:stb-mi}); local convergence acceleration through spectral preconditioning (Theorem~\ref{thm:stb-conv}); the first non-vacuous PAC-Bayes generalization bound for 7B-scale PEFT, with effective dimension $d_{\text{eff}} \approx 1{,}760$ and $\KL \leq 7{,}320$ (Theorem~\ref{thm:pacbayes}); exact gradient preservation under spectral compression (Theorem~\ref{thm:sgc}); linear-in-epoch error accumulation for progressive ODE integration (Theorem~\ref{thm:nfr}); and provably non-zero mutual information under stochastic STB bypass (Theorem~\ref{thm:sbs}).
