...
\begin{itemize}[leftmargin=*]
    \item \textbf{Statistical power.} \Cref{tab:pathways} (ablations) and LoRA's numbers in \Cref{tab:main} have two-seed data (42, 43) and agree closely across seeds (\Cref{sec:stats}). \sthree{}'s own downstream numbers in \Cref{tab:main} remain single-seed: its second-seed checkpoint trained successfully but the seven-task evaluation was cut short by a hardware failure and had not been re-run at the time of writing. \sthree{}'s specific margin against LoRA should be read as a one-run estimate until that second seed is measured.
...
Empirically (\Cref{sec:experiments}), on seven zero-shot lm-eval-harness tasks, \sthree{} reaches downstream accuracy within a few points of LoRA rank-8 while training $5.2\times$ fewer parameters. An ablation over coupled operator pathways shows the spectral pathway (SVMO+STB) matching the flow pathway (NMF) with 15\% fewer parameters at low budget, and combining both beating either alone at matched total budget. A solver-invariance sweep supports that NMF's behavior barely depends on the RK4 step count it was trained under, evidence of a genuine continuous flow rather than a step-count-specific shortcut. The ablation and LoRA comparisons above are backed by two-seed data (\Cref{sec:stats}); \sthree{}'s own downstream margin against LoRA in \Cref{tab:main} is still a single run, pending its second-seed evaluation, and should be read as such. \sthree{}'s intended audience is stated explicitly in \Cref{sec:motivation}: users for whom on-demand cloud GPUs are not actually a substitute. Code, pre-computed SVD factors, and experimental configurations will be released alongside publication.
