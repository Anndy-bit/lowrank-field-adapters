...
% IEEE journal format. Data traceability: see OUTLINE.md.
...
% Fig. 3 palette: a visit either pays a disk read or finds the unit resident.
...
% IEEE style, and IEEEtran's own captions, use "Fig." — cleveref defaults to
% "Figure", which would disagree with the caption it points at.
...
% Bloque de autor: se deja SIN autor a propósito. Descomentar y rellenar antes del envío final.
% \author{Nombre Apellido%
% \thanks{Afiliación, ciudad, país. Correo: usuario@dominio.}%
% \thanks{Manuscript prepared July 2026. All experiments were carried out on a single
% NVIDIA GTX~1050 (3.94\,GB) desktop machine.}}
\author{\thanks{Manuscript prepared July 2026. All experiments were carried out on a single
NVIDIA GTX~1050 (3.94\,GB) desktop machine.}}
...
the frozen base must remain resident throughout the forward and backward passes. We show that this
requirement is an artifact of implementation rather than a mathematical necessity. Reverse-mode
...
(\Cref{thm:exact}) rather than approximate. The consequence is a memory bound independent of model
depth and of total parameter count: only the largest single unit matters (\Cref{thm:vram}). We
further prove that inverting the loop order (\emph{layer-major}, \Cref{thm:lm}) divides disk
traffic by the gradient-accumulation factor $G$ while leaving gradients bit-identical, shifting the
regime from I/O-bound to compute-bound. On a GTX~1050 (3.94\,GB) we fine-tune Qwen2.5-7B---15.2\,GB
...
speed-up. A predictive cost model matches measurements within 6\%. \usf{} is adapter-agnostic: a
spectral adapter and LoRA both run unmodified. Under the same budget, 4-bit quantization does not
fit (5.43\,GB required), making this a feasibility separation rather than a preference. The bound
predicts that Llama-3-70B fits the same card at the implemented granularity; we state precisely
what this does and does not establish.
\end{abstract}

\begin{IEEEkeywords}
Parameter-efficient fine-tuning, memory-constrained training, large language models, exact
gradients, out-of-core computation, automatic differentiation, consumer hardware.
\end{IEEEkeywords}
...
\IEEEPARstart{P}{arameter-efficient} fine-tuning (PEFT) reduced the \emph{trainable} footprint of
...
designed for a different regime---training \emph{all} weights across a cluster---where the dominant
...
(\Cref{thm:exact}) and verify ($\max|\Delta|=0$) that the gradients are identical to those of
...
the largest \emph{single} unit, not by the model:
\begin{equation}
\label{eq:headline}
...
(\Cref{thm:vram} and its corollaries). Adding layers to a model does not increase its memory
requirement; it increases only time and disk traffic. Model size ceases to be a memory problem and
becomes a storage-and-time problem. We regard this decoupling, rather than any individual
engineering component, as the contribution of this work.
...
divides disk traffic by $G$ with bit-identical gradients (\Cref{thm:lm}). This reordering is
...
parameter count (\Cref{thm:vram}, \Cref{cor:depth}, \Cref{cor:size}).
\item \textbf{Exactness, proved and verified.} \usf{} preserves gradients exactly
(\Cref{thm:exact}); we measure $\max|\Delta|=0$, including on the 7B model itself
(\Cref{sec:exp}).
\item \textbf{Layer-major streaming.} Exchanging the loop order divides I/O by the accumulation
factor $G$ at no cost in correctness (\Cref{thm:lm}), changing the performance regime
(\Cref{sec:cost}).
\item \textbf{A predictive cost model} that matches measurement within 6\% (\Cref{sec:cost}),
which is what licenses our scaling statements as predictions rather than extrapolations.
\item \textbf{An adapter- and model-agnostic system.} The same infrastructure runs LoRA and a
spectral adapter unmodified (\Cref{cor:agnostic}), and selects its streaming granularity
automatically from the available memory (\Cref{sec:auto}).
\end{enumerate}

We are explicit about scope. We \emph{demonstrate} 7B on a 4\,GB card; we \emph{predict}, from a
...
(\Cref{sec:scaling}). We do not claim the latter two as results.
...
\Cref{cor:agnostic} makes this formal. We evaluate LoRA and a spectral adapter under \usf{} without
modifying either.
...
is exact (\Cref{cor:noapprox}). Second, it improves the constant but preserves the coupling to
model size; \Cref{sec:quant} reports the concrete consequence, namely that 4-bit is infeasible on
...
(\Cref{thm:vram}), (ii) the granularity hierarchy that determines feasibility
(\Cref{thm:gran}), and (iii) the layer-major reordering (\Cref{thm:lm}), which has no purpose in a
...
at inference; \Cref{sec:quant} reports our measurements with that path.
...
substantial gains, and that exactness is worth preserving rather than trading away. \usf{} shares
...
device for the weights of the entire model. \Cref{thm:lm} is an I/O-aware scheduling result in the
same spirit, and \Cref{sec:cost} follows the roofline reasoning of~\cite{williams2009roofline} in
...
the pipeline. Superficially, layer-major (\Cref{thm:lm}) resembles a pipeline schedule; the purpose
...
\Cref{alg:usf} states the base algorithm. The forward pass builds no autograd graph: activations
...
$\nabla_\theta \ell^{\usf}$ be the gradient produced by \Cref{alg:usf}. Then, in exact arithmetic,
\begin{equation}
...
\Cref{alg:usf} supplies all three unchanged. For (a), the checkpoint $c_i = h_{i-1}$ is recorded
...
full graph, and their composition---which is precisely the chain rule---yields the same value.
Finally, $\textsc{Free}(W_i)$ alters only where a tensor resides, not its value; arithmetic is
invariant to device placement.
\end{IEEEproof}
...
confined to the ordinary non-determinism of floating-point arithmetic (\Cref{rem:fp}).
\end{corollary}

\begin{corollary}[Adapter agnosticism]
\label{cor:agnostic}
\Cref{thm:exact} uses no property of $f_i$ beyond differentiability. \usf{} is therefore
independent of the adapter: it applies verbatim to LoRA, to spectral adapters, or to any
parameterisation of $\theta$. We verify this with two different adapters in \Cref{sec:exp}.
\end{corollary}

\begin{remark}[Floating-point considerations]
\label{rem:fp}
\Cref{thm:exact} asserts equality in exact arithmetic; two points deserve care in practice. First,
...
\label{thm:vram}
Under \Cref{alg:usf} with granularity $\mathcal{U}$, the peak device memory satisfies
\begin{equation}
\label{eq:vram}
...
\begin{IEEEproof}
At every instant of \Cref{alg:usf} exactly one unit is loaded, since $\textsc{Load}$ and
$\textsc{Free}$ are strictly nested and non-overlapping (\Cref{def:ops}); this yields the first
...
\label{cor:depth}
The checkpoints $c_i$ are activations, not weights, and are not required on device between their
production and their use. Offloading them to host memory gives $K \approx 0$ and hence
\begin{equation}
\mathrm{VRAM}_{\text{peak}} \;\approx\; \max_u \Wu \cdot s + |\theta| \cdot s_{\text{opt}} + A ,
\end{equation}
...
\Cref{cor:size} is the paper's central claim, and it is worth stating what it does and does not
say. It does not say that a larger model is as cheap to fine-tune as a smaller one: the I/O and
compute costs both scale with $\sum_i |W_i|$, as \Cref{sec:cost} quantifies. It says that those
...
\label{thm:gran}
For the nested partitions of \Cref{def:unit},
\begin{equation}
\max_{i,p} |W_{i,p}| \;\le\; \max_i \max\!\left(|W_i^{\text{attn}}|, |W_i^{\text{mlp}}|\right)
\;\le\; \max_i |W_i| ,
\end{equation}
and \Cref{thm:vram} applies to any of them. Refining the granularity reduces peak memory
...
\Cref{tab:scaling} instantiates \Cref{thm:gran} for four model families. Two observations follow.
...
granularity, but its largest single matrix occupies 1.62\,GB and would fit---which is the content
of \Cref{cor:size} at its extreme.

\begin{remark}[An implementation limit, stated plainly]
\label{rem:matrixlimit}
\Cref{thm:gran} is a statement about partitions and holds at matrix granularity. Our
\emph{implementation}, however, supports layer and sublayer granularity only. The reason is
engineering rather than mathematics: we delegate attention to the reference
...
granularity, and we claim nothing for 405B beyond \Cref{tab:scaling}.
\end{remark}
...
\Cref{alg:usf} is \emph{batch-major}: for each micro-batch it traverses all $L$ units. With
...
\Cref{fig:schedule} shows why this matters. The two schedules visit exactly the same set of
(unit, micro-batch) pairs, and by \Cref{thm:exact} each visit computes the same local VJP; they
...
\end{tikzpicture}
\caption{Why inverting the loop order divides I/O by $G$ (\Cref{thm:lm}), drawn for $L{=}3$ units
...
boundary activations, which \Cref{cor:depth} places in host memory.}
\label{fig:schedule}
\end{figure}

\begin{theorem}[Exactness and I/O of layer-major]
\label{thm:lm}
The layer-major schedule of \Cref{def:lm} produces gradients identical to \Cref{alg:usf}, and its
...
$\nabla_\theta \ell^{(j)}$ is computed by the local VJP of \Cref{thm:exact}, evaluated at
$(c_i^{(j)}, W_i, \theta_i)$---the same points as in the batch-major schedule. Exchanging the loops
changes the order of visits, not any evaluation point.

\emph{I/O.} In \Cref{def:lm} the outer loop traverses $i$ once per optimizer step, so each $W_i$ is
loaded exactly once per pass and twice in total (forward and backward), giving $2\sum_i |W_i|$. In
\Cref{alg:usf} the outer loop over $j$ repeats the full traversal $G$ times.
\end{IEEEproof}
...
59\,MB per boundary, and by \Cref{cor:depth} it may be placed in host memory. The trade is
therefore $G$-fold less I/O in exchange for $G$-fold more offloadable activations.

\begin{proposition}[Choice of $G$]
\label{prop:G}
Under layer-major, the time per example is
\begin{equation}
T_{\text{ex}}(G) = \frac{2\sum_i |W_i|}{\mathrm{BW}\cdot G\cdot B} + \frac{C}{B},
\end{equation}
with $C$ the compute time of one micro-batch and $\mathrm{BW}$ the disk bandwidth. $T_{\text{ex}}$
is monotonically decreasing in $G$ and converges to the compute-bound limit $C/B$. The largest $G$
satisfying the activation-memory constraint is therefore optimal.
\end{proposition}
...
remains comfortable on a 3.94\,GB card. With our calibrated constants (\Cref{sec:cost}), one full
...
$8.6\times10^{2}$ tokens and this configuration supplying $1024$. Combined with \Cref{thm:lm},
which divides $t_{\text{io}}$ by $G$, the margin becomes ample. Prefetching alters when a tensor is read, never its value, so
\Cref{thm:exact} is unaffected---a fact we verify rather than assume (\Cref{tab:exact}).

%==============================================================================
\section{Cost Model}
\label{sec:cost}

Let $N = \sum_i |W_i| / s$ be the number of streamed parameters, $\mathrm{BW}$ the effective disk
bandwidth, and $\Phi$ the effective device throughput. Using the standard estimate of $6N$ FLOPs
...
$\max(T_{\text{io}}, T_{\text{cmp}})$ with it (\Cref{prop:prefetch}). The system is compute-bound
...
not of the model's size, so a larger model does not become more I/O-bound---it becomes uniformly
...
magnitude---the reordering of \Cref{thm:lm} does not merely accelerate the system, it changes which
...
($B=1$ and $B=4$) and used the model to predict the remainder. \Cref{tab:cost} reports the outcome:
the largest discrepancy is 6\%. We rely on this agreement in \Cref{sec:scaling}, where the model is
used predictively.
...
\Cref{thm:vram,thm:gran} yield an operative criterion. Given a memory budget $V$, traverse the
...
\begin{IEEEproof}
Total I/O is invariant to granularity (\Cref{thm:gran}), but the number of $\textsc{Load}$
...
no supported granularity fits, the system reports this rather than failing opaquely---and, per
\Cref{rem:matrixlimit}, it states explicitly when an unimplemented granularity would have
sufficed.
...
All experiments run on a single desktop with an NVIDIA GTX~1050 (3.94\,GB usable), 8 CPU cores,
15.5\,GB of host memory, and an NVMe disk. The model is
...
\Cref{cor:agnostic}: LoRA~\cite{hu2022lora} at ranks 1, 2 and 8, and \sthree{}, a
...
measurements reported here. Each exactness claim in \Cref{tab:exact} corresponds to an executable
test, and the figures are regenerated from the logged run rather than redrawn, so the reported
deviations can be reproduced rather than taken on trust.

\subsection{Exactness}

\Cref{thm:exact}, \Cref{thm:lm}, \Cref{thm:gran} and \Cref{prop:prefetch} each assert that a
transformation leaves gradients unchanged. \Cref{tab:exact} reports the measured deviation for
...
\midrule
\Cref{thm:exact}     & streamed vs.\ resident        & 150  & $0.00$ \\
\Cref{thm:lm}        & layer- vs.\ batch-major (7B)  & 1400 & $0.00$ \\
\Cref{cor:depth}     & checkpoints on host vs.\ device & 150  & $0.00$ \\
\Cref{thm:gran}      & sublayer vs.\ layer           & 150  & $0.00$ \\
\Cref{prop:prefetch} & prefetch on vs.\ off          & 150  & $0.00$ \\
\bottomrule
\end{tabular}
\end{table}

The second row deserves emphasis. \Cref{thm:exact} can only be verified against a resident
baseline on a model that \emph{fits} resident, which on this hardware means a small one; we
therefore verify it on a reduced configuration. \Cref{thm:lm}, by contrast, compares two streaming
...
\Cref{tab:vram} reports peak device memory. Both adapters train the same frozen 7B model on the
same card. The gap between them is precisely the $|\theta|\cdot s_{\text{opt}} + A$ term of
\eqref{eq:vram}---\sthree{} keeps additional resident factors---and both satisfy the bound. That
LoRA occupies \emph{less} memory than \sthree{} is not a defect of the system but a confirmation of
\Cref{cor:agnostic}: \usf{} imposes its bound irrespective of what is being adapted.
...
\emph{resv} and \emph{smi}---because allocator fragmentation, not parameter count, dominates these
two measures at this scale; \Cref{thm:vram} bounds the allocation, not the allocator's caching
policy.

\subsection{Learning end to end}

\Cref{tab:vram} establishes that training reduces held-out perplexity for every adapter. A longer
...
no step skipped due to gradient overflow. \Cref{fig:loss} plots the trajectory. We report these as
evidence that the system trains, not as a quality claim: by \Cref{thm:exact} the resulting model is
the model that ordinary training would have produced, so there is nothing about quality that
\usf{} could change.

\Cref{fig:vram} shows the device-memory trace of that run, and is the most direct empirical
picture of \Cref{thm:vram}. The sawtooth is the mechanism itself: each tooth is one
...
blocks, which is why we report all three measures in \Cref{tab:vram} rather than the most
flattering one.
...
The sawtooth is \Cref{alg:usf} itself: each tooth is one $\textsc{Load}$--compute--$\textsc{Free}$
cycle of a 0.43\,GB decoder layer. The model is 15.2\,GB on disk, yet nothing approaches the
card's 4034\,MB---the picture \Cref{thm:vram} predicts. The upward drift of the reserved curve is
...
11.96 to 5.50. No step was skipped for gradient overflow. By \Cref{thm:exact} this trajectory is
the one ordinary residency training would have produced; we show it as evidence that the system
trains, not as a quality claim.}
\label{fig:loss}
\end{figure}

\subsection{Layer-major}

\Cref{tab:lm} compares the two schedules on the 7B model over 16 examples with $G=4$. Layer-major
is $1.40\times$ faster and uses \emph{less} memory, the latter because its checkpoints are
host-resident (\Cref{cor:depth}). \Cref{prop:G} predicts further gains at larger $G$; we did not
measure them.
...
asymmetry does not matter, which is why we can state the conclusion as a separation of feasibility
rather than a preference. The two techniques are composable and address different axes:
quantization purchases memory with accuracy, \usf{} purchases it with time, and only the latter is
exact (\Cref{cor:noapprox}).

%==============================================================================
\section{Scaling Analysis}
\label{sec:scaling}

\Cref{tab:claims} states what this paper establishes at each scale, and on what basis. We consider
the distinction important enough to tabulate rather than to leave to prose.
...
(\Cref{tab:scaling}); the weights were not available to us \\[2pt]
Llama-3-405B &
\textbf{Theoretical; requires an unimplemented extension} &
Needs matrix granularity (1.62\,GB); see \Cref{rem:matrixlimit} \\
\bottomrule
\end{tabular}
\end{table}

Using \eqref{eq:tio}--\eqref{eq:tcmp} with the constants calibrated in \Cref{sec:cost}, and
...
hardware is 7B--13B, and we say so in \Cref{sec:limits} rather than leaving it to be discovered.
...
method is a trade, and \Cref{sec:scaling} quantifies it. We claim feasibility on hardware where the
...
\textbf{The PEFT hypothesis is load-bearing.} \Cref{thm:vram} treats $|\theta|\cdot s_{\text{opt}}$
as small. With many trainable parameters the optimizer state ceases to be negligible and
partitioning it becomes the dominant concern---the regime of ZeRO~\cite{rajbhandari2020zero}, not
of this paper. \usf{} is a method for PEFT, not for pre-training.

\textbf{Matrix granularity is not implemented} (\Cref{rem:matrixlimit}), which is why
\Cref{tab:claims} assigns 405B a weaker status than 70B.

\textbf{Constants are hardware-specific.} The cost \emph{model} of \Cref{sec:cost} is general; the
values of $\Phi$ and $\mathrm{BW}$ used in \Cref{tab:cost,tab:claims} are not, and would need
...
differentiation is local and residency is therefore a matter of scheduling rather than of
correctness. The resulting method is exact---verified at zero deviation on the 7B model itself---
adapter-agnostic, and governed by a cost model accurate to within 6\%. Its consequence is a
displacement rather than an elimination of cost: model size becomes a question of time and storage.
...
card. What remains scarce is time, and time is a more tractable adversary than a memory wall.
