...
(\Cref{thm:exact}) and not approximate. The consequence is a memory bound independent of model
...
which is why we treat the scaling statements in \Cref{sec:scaling} as predictions rather than
extrapolations.
...
\Cref{cor:size} is the paper's central claim. It does not say that a larger model is as cheap to
...
granularity, but its largest single matrix occupies 1.62\,GB and would fit: this is \Cref{cor:size}
...
(\Cref{sec:cost} quantifies the gap). $G$ appears only in the I/O term, in the denominator, so
...
magnitude. The reordering of \Cref{thm:lm} does not just accelerate the system; it changes which
...
\Cref{rem:matrixlimit}, it also states explicitly when an unimplemented granularity would have
sufficed.
...
point---if the bound in \Cref{thm:vram} holds here, it holds on any GPU a practitioner without
...
LoRA occupies \emph{less} memory than \sthree{} is exactly what \Cref{cor:agnostic} predicts, not a
...
measures far more than parameter count does. \Cref{thm:vram} bounds the allocation itself; the
...
card's 4034\,MB, exactly the picture \Cref{thm:vram} predicts. The upward drift of the reserved curve is
...
11.96 to 5.50, with no step skipped for gradient overflow. By \Cref{thm:exact}, \usf{} reproduces
...
host-resident (\Cref{cor:depth}). \eqref{eq:choiceG} predicts further gains at larger $G$; we did
not measure them.
...
\Cref{tab:claims} states what this paper establishes at each scale, and on what basis.
...
card. The barrier that remains is time, which \Cref{sec:scaling} quantifies, and which---unlike
