%==============================================================================
% USF — Ajuste Fino por Streaming de Unidades. Artículo 1 (el sistema de streaming).
...
\title{\usf{}: Ajuste Fino por Streaming de Unidades con Gradientes\\ Exactos en GPUs de Consumo}
...
% equipo de escritorio con una NVIDIA GTX~1050 (3,94\,GB).}}
\author{\thanks{Manuscrito preparado en julio de 2026. Todos los experimentos se realizaron en un único
equipo de escritorio con una NVIDIA GTX~1050 (3,94\,GB).}}
...
%==============================================================================
\begin{abstract}
El ajuste fino de un modelo de lenguaje de 7\,B parámetros requiere convencionalmente entre 8 y
16\,GB de memoria de GPU, dado que la base congelada debe permanecer residente durante los pases
hacia adelante y hacia atrás. Demostramos que este requisito es un artefacto de la implementación
...
el \emph{Ajuste Fino por Streaming de Unidades} (\usf), que mantiene exactamente una unidad de
pesos residente a la vez, transmitida desde disco, y demostramos que los gradientes resultantes son
\emph{exactos} (\Cref{thm:exact}) y no aproximados. La consecuencia es una cota de memoria
...
limitado por E/S a estarlo por cómputo. En una GTX~1050 (3,94\,GB) ajustamos Qwen2.5-7B ---15,2\,GB
...
(requiere 5,43\,GB), lo que convierte esto en una separación de viabilidad y no en una preferencia.
La cota predice que Llama-3-70B cabe en la misma tarjeta con la granularidad implementada;
precisamos con exactitud qué establece y qué no establece dicha afirmación.
\end{abstract}

\begin{IEEEkeywords}
Ajuste fino eficiente en parámetros, entrenamiento con memoria limitada, modelos de lenguaje de
gran tamaño, gradientes exactos, cómputo fuera de núcleo, diferenciación automática, hardware de
consumo.
\end{IEEEkeywords}
...
\IEEEPARstart{E}{l} ajuste fino eficiente en parámetros (PEFT) redujo en órdenes de magnitud la
huella \emph{entrenable} de la adaptación de modelos de lenguaje: LoRA~\cite{hu2022lora} actualiza
muy por debajo del 1\,\% de los pesos, y los adaptadores~\cite{houlsby2019adapters} o el ajuste de
prompts~\cite{lester2021prompt} actualizan aún menos. La huella \emph{residente}, sin embargo, no
...
consumo, y la compresión se paga en precisión. Los marcos de descarga como
...
de productos vector--jacobiano (VJP) \emph{locales}. Para derivar a través de la capa $i$ se
...
fija la unidad individual más grande, no el modelo:
\begin{equation}
\label{eq:headline}
...
Somos explícitos sobre el alcance. \emph{Demostramos} 7\,B en una tarjeta de 4\,GB;
...
\textbf{Ajuste fino eficiente en parámetros.} LoRA~\cite{hu2022lora} y sus
descendientes~\cite{liu2024dora}, los adaptadores~\cite{houlsby2019adapters} y el ajuste de
prompts~\cite{lester2021prompt} minimizan los parámetros \emph{entrenables}. Son ortogonales a este
...
\textbf{Descarga y particionado.} ZeRO~\cite{rajbhandari2020zero},
...
problema se reduce por completo a la residencia de $W$. Lo que aportamos no es, por tanto, la
descarga en sí, sino (i) la cota formal y su independencia del tamaño del modelo
...
\textbf{Segmentación en cauce.} GPipe~\cite{huang2019gpipe} y enfoques
relacionados~\cite{jia2019beyond} reparten capas entre dispositivos y solapan micro-lotes para
llenar el cauce. Superficialmente, layer-major (\Cref{thm:lm}) recuerda a una planificación de
cauce; el propósito está invertido. La segmentación en cauce distribuye un modelo demasiado grande
para un dispositivo entre varios; layer-major conserva un único dispositivo y amortiza el coste de
\emph{releer} el modelo, un gasto que solo existe cuando los pesos se transmiten.
...
Finalmente, $\textsc{Liberar}(W_i)$ altera únicamente dónde reside un tensor, no su valor; la
aritmética es invariante a la ubicación en el dispositivo.
\end{IEEEproof}
...
recomputación selecciona un núcleo distinto del pase hacia adelante original, el orden de reducción
en punto flotante puede diferir en los últimos bits. Este es el no determinismo inherente al
checkpointing de activaciones estándar~\cite{chen2016checkpointing} y no es específico de \usf{}.
En nuestras mediciones, con formas y núcleos idénticos, la concordancia es exacta
(\Cref{tab:exact}).
\end{remark}
...
\label{cor:depth}
Los checkpoints $c_i$ son activaciones, no pesos, y no se requieren en el dispositivo entre su
producción y su uso. Descargarlos a la memoria del anfitrión da $K \approx 0$ y por tanto
\begin{equation}
\mathrm{VRAM}_{\text{pico}} \;\approx\; \max_u \Wu \cdot s + |\theta| \cdot s_{\text{opt}} + A ,
\end{equation}
...
El \Cref{cor:size} es la afirmación central del artículo, y conviene precisar qué dice y qué no.
No dice que un modelo mayor sea tan barato de ajustar como uno menor: los costes de E/S y de
...
\begin{remark}[Un límite de implementación, expuesto sin rodeos]
...
los bucles cambia el orden de las visitas, no ningún punto de evaluación.
...
intercambio es, por tanto, $G$ veces menos E/S a cambio de $G$ veces más activaciones descargables.

\begin{proposition}[Elección de $G$]
\label{prop:G}
Bajo layer-major, el tiempo por ejemplo es
\begin{equation}
T_{\text{ej}}(G) = \frac{2\sum_i |W_i|}{\mathrm{BW}\cdot G\cdot B} + \frac{C}{B},
\end{equation}
con $C$ el tiempo de cómputo de un micro-lote y $\mathrm{BW}$ el ancho de banda de disco.
$T_{\text{ej}}$ es monótonamente decreciente en $G$ y converge al límite de cómputo $C/B$. Es
óptimo, por tanto, el mayor $G$ que satisfaga la restricción de memoria de activaciones.
\end{proposition}
...
\begin{IEEEproof}
La planificación es un cauce de dos etapas (carga; cómputo) con búferes independientes. En régimen
...
Sean $N = \sum_i |W_i| / s$ el número de parámetros transmitidos, $\mathrm{BW}$ el ancho de banda
efectivo de disco y $\Phi$ el rendimiento efectivo del dispositivo. Empleando la estimación
...
razonamiento roofline de~\cite{williams2009roofline} aplicado a un bucle de entrenamiento fuera de
núcleo.
...
\begin{proposition}[Optimalidad del ajuste más grueso]
\label{prop:auto}
...
Todos los experimentos se ejecutan en un único equipo de escritorio con una NVIDIA GTX~1050
(3,94\,GB utilizables), 8 núcleos de CPU, 15,5\,GB de memoria de anfitrión y un disco NVMe. El
...
espectral-espacial-suave descrito en un artículo complementario. Este trabajo no formula
...
\Cref{cor:depth}     & checkpoints en anfitrión vs.\ dispositivo & 150 & $0{,}00$ \\
\Cref{thm:gran}      & sub-capa vs.\ capa                & 150  & $0{,}00$ \\
\Cref{prop:prefetch} & prefetch activado vs.\ desactivado & 150  & $0{,}00$ \\
\bottomrule
\end{tabular}
\end{table}
...
\emph{resv} y \emph{smi}--- porque a esta escala domina la fragmentación del asignador y no el
número de parámetros; el \Cref{thm:vram} acota la asignación, no la política de caché del
asignador.
...
el entrenamiento ordinario con residencia; la mostramos como evidencia de que el sistema entrena,
no como una afirmación de calidad.}
\label{fig:loss}
\end{figure}
...
checkpoints residen en el anfitrión (\Cref{cor:depth}). La \Cref{prop:G} predice mayores ganancias
con $G$ superior; no las medimos.
...
Consideramos la distinción lo bastante importante como para tabularla en lugar de dejarla en prosa.
...
despreciable y su particionado pasa a ser la preocupación dominante: el régimen de
ZeRO~\cite{rajbhandari2020zero}, no el de este artículo. \usf{} es un método para PEFT, no para
preentrenamiento.
...
modo inverso es local y la residencia es, por tanto, una cuestión de planificación y no de
corrección. El método resultante es exacto ---verificado con desviación nula sobre el propio modelo
de 7\,B---, agnóstico al adaptador y gobernado por un modelo de coste preciso dentro de un 6\,\%.
...
una granularidad implementada, sitúa un modelo de 70\,B al alcance de esa misma tarjeta. Lo que
sigue siendo escaso es el tiempo, y el tiempo es un adversario más tratable que un muro de memoria.
